...
sub new {
    ...
    #$$self{'SVNUSER'} = "Anonymous";
    #$$self{'SVNPASS'} = "Anonymous";
...
sub GatherFileInformation {
    ...
    if ( ! open($hProcess, "svn.exe info -R \"$SourceRoot\"|") ) {
        ::warn_message("Unable to start svn.exe: $!");
        return();
    }

    ::status_message("Processing svn.exe properties output ...");
...
sub SourceStreamVariables {
    ...
    if (defined $$self{'SVNUSER'})
    {
    push(@stream, "SVN_EXTRACT_CMD=cmd.exe /c svn.exe cat ".
                                   "\"%var2%%var3%@%var4%\" --revision %var4% ".
...
SVN specific settings:
  ...
  -----------------------------------------------------------------------------
  A) SVN Revision    Rev         SVNREV          <N/A>
  A) SVN Username    User        SVNUSER         Anonymous
  A) SVN Password    Pass        SVNPASS         Anonymous
...
(A)  SVN Revision - Causes the script to replace individual file versions with the
     given revision.  The script does not verify that the revision exists.
(B)  SVN Username - The username used to logon into SVN when the source files are
     exported from the repository by the debugger.
(C)  SVN Password - The password used to logon into SVN when the source files are
     exported from the repository by the debugger.
SVN_VERBOSE_USAGE
}
