        ...
        if (substr($_, 0, 1) =~ /^[\/-]$/) {
            ...
            if ( (@opt = split(/=/, $_))==2 ) {
                block: {
                    ...
                    # -SYSTEM=<string> (source contorl ssytem to use)
...
foreach (@SourceRoots) {
    s/^\s*(.*)\s*$/$1/;
    $Options{'SRCSRV_PROVIDER'}->GatherFileInformation($_, \%ServerVars);
}
...
if ( defined $Options{'SRCSRV_SAVE'} ) {
    ...
    my $fh;
    if ( open($fh, ">$Options{'SRCSRV_SAVE'}\\servers.dat") ) {
...
sub IndexThisPdb {
    ...
    if ( $filecount > 0 ) {
        ...
        chomp($result);
        if ( $result =~ /error/i ) {
            info_message("... error writing $filename to $pdb ($result)");
        } else {
...
sub LoadServerVars {
    ...
    while ( defined($line = <$fh>) && ($line !~ m/^\s*\[.*\]/i) ) {
        ...
        my ($Token,$Server) = split(/\s*=\s*/, $line);
...
sub Usage {
    ...
    print '-'x80,"\n";
    print("$ScriptName [/option=<value> [...]] [ModuleOptions] [/Debug]\n");
    print("\n");
...
sub GetTempFile {
    ...
    } while ( !sysopen($fh, $path, O_RDWR|O_CREAT|O_EXCL) &&
              ($RetryCount++ < 10 ) );
